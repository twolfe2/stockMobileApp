...
export default (api) => {
  ...
  function * worker (userInput) {
    // make the call to the api
    const response = yield call(api.stockLookup, userInput)
    console.log('response:', response);
    // success?
    if (response.ok) {
      yield put(Actions.receiveSymbols(response.data))

     } else {
      yield put(Actions.receiveSymbolsFailure())
    }
  }
  ...
  function * watcher () {
    while (true) {
      const action = yield take(Types.SYMBOLS_REQUEST)
      console.log('in saga stock info!!!')
      const { userInput } = action
      console.log('userInput', userInput)
      yield call(worker, userInput)
    }
  }
