...
export default (api) => {
  ...
  function * worker (symbol) {
    ...
    const response = yield call(api.getStockInfo, symbol)
    console.log('response:', response);
    // success?
    if (response.ok) {
      yield put(Actions.receiveStockInfo(response.data))

     } else {
      yield put(Actions.receiveStockInfoFailure())
    }
  }
  ...
  function * watcher () {
    while (true) {
      const action = yield take(Types.SYMBOL_INFO_REQUEST)
      const { symbol } = action
